"""High-level GeoPackage (GPKG) import to EverGIS layer."""
...
def import_gpkg_to_layer(
    ...
    *,
    ...
    """Import one layer of a GeoPackage into an EverGIS layer.

    A GPKG is a container - every call imports a single ``source_layer_name``.
    Iterate over multiple layers in the caller to load the whole file.

    Args:
        client: Initialized EverGIS API client.
        source_file_name: Catalog path / resource ID / system name of the
            GPKG file already uploaded into EverGIS.
        source_layer_name: Layer name inside the GPKG.
        ...
        log: Verbose output.
        defer: When True, **do not** submit the task. Return a
            :class:`~evergis_tools.tasks.pipeline.TaskStep` instead,
            ready to drop into a :class:`~evergis_tools.tasks.pipeline.TaskPipeline`
            for batching with other subtasks under a single prototype.

    Returns:
        ``TaskExecutionResult`` (default) or a ``TaskStep`` when
        ``defer=True``.
    """
    ...
    start_params = ImportexportSourceGpkgTargetLayerStartParameters(
