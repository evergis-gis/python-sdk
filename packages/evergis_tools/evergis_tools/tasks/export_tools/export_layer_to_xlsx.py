"""High-level export of EverGIS layer to XLSX format."""
...
def export_layer_to_xlsx(
    ...
    *,
    ...
    """Export EverGIS layer to XLSX file via ImportExport task.
    ...
    Args:
        ...
        target_file_name: Resource ID or catalog path for target XLSX file.
    ...
    Example:
        ...
        ...     target_file_name="filtered_export.xlsx",
        ...     target_parent_path="john_doe:Projects/Exports",
        ...     batch_count=50000
        ... )
    ...
    # If parent path is provided, resolve it first, then resolve target within parent
    if target_parent_path:
        # Resolve parent folder (ensures it exists)
        resolve_resource(client, target_parent_path)
        # Use target_file_name as-is and let the task create it in parent folder
        target_file_resource_id = target_file_name
    else:
        # Resolve target file resource directly
    ...
    start_params = LayerToExcelStartParameters(
